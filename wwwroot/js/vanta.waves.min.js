...
, i.t=function(t,e) {
    ...
    if(i.r(s),Object.defineProperty(s,"default",{enumerable:!0,value:t}),2&e&&"string"!=typeof t)for(var o in t)i.d(s,o,function(e){return t[e]}.bind(null,o));
    return s
}
...
;try {
    ...
    return i=t,n("Init error"),n(i),this.el.removeChild(this.renderer.domElement),void(this.options.backgroundColor&&(console.log("[VANTA] Falling back to backgroundColor"),this.el.style.background=Object(s.a)(this.options.backgroundColor)))
}
...
{
    color: this.options.color,shininess:this.options.shininess,flatShading: !0,vertexColors:THREE.FaceColors,side:THREE.DoubleSide
}
...
onInit() {
    let t,e;
    const i=this.getMaterial(),s=new THREE.Geometry;
    for(this.gg=[],t=0;
    t <=this.ww;
    t ++)for(this.gg[t]=[],e=0;
    e <=this.hh;
...
{
    const i=s.vertices.length,n=new THREE.Vector3(18*(t-.5*this.ww),Object(o.g)(0,this.waveNoise)-10,18*(.5*this.hh-e));
    s .vertices.push(n),this.gg[t][e]=i
}

for(t=1;t<=this.ww;t + + )for(e=1;e<=this.hh;e + + ) {
    var n,r;
    const i=this.gg[t][e],h=this.gg[t][e-1],a=this.gg[t-1][e],l=this.gg[t-1][e-1];
    Object(o.f)(0,1)?(n=new THREE.Face3(l,h,a),r=new THREE.Face3(h,a,i)):(n=new THREE.Face3(l,h,i),r=new THREE.Face3(l,a,i)),s.faces.push(n,r)
}

this.plane=new THREE.Mesh(s,i), this.scene.add(this.plane);const h=new THREE.AmbientLight(16777215,.9);this.scene.add(h);const a=new THREE.PointLight(16777215,.9);a.position.set(-100,250,-100), this.scene.add(a), this.camera=new THREE.PerspectiveCamera(35,this.width/this.height,50,1e4);this.cameraPosition=new THREE.Vector3(240,200,390), this.cameraTarget=new THREE.Vector3(140,-30,190), this.camera.position.copy(this.cameraPosition), this.scene.add(this.camera)
}
...
onUpdate() {
    ...
    for(let t=0;
    t <this.plane.geometry.vertices.length;
    t ++)
...
{
    const t=this.options.waveSpeed,i=Math.sqrt(t)*Math.cos(-e.x-.7*e.z),s=Math.sin(t*this.t*.02-t*e.x*.025+t*e.z*.015+i),o=Math.pow(s+1,2)/4;
...
n.prototype.defaultOptions= {
    color: 21896,shininess:30,waveHeight:15,waveSpeed:1,zoom:1
}
